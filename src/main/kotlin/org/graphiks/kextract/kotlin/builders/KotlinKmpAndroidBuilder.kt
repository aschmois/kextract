...
internal class KotlinKmpAndroidBuilder(
    ...
    override fun visitScoped(decl: Declaration.Scoped) {
        ...
        when (decl.kind()) {
            ...
            Declaration.Scoped.Kind.UNION -> {
                ...
                builder.indent()
                // Adopting a pointer wgpu already filled in: JNA only materializes a
                // Structure's Java fields on read(), so skipping it hands back zeros.
                builder.appendLine("return ByReference($rawJnaByReference(address).also { it.read() })")
                builder.unindent()
                builder.appendLine("}")
                builder.appendLine()
                ...
                // JNA Memory starts as uninitialized heap; descriptors only assign the fields the
                // caller sets, so anything left unset would reach wgpu as garbage.
                builder.appendLine("ref.clear()")
                ...
                builder.indent()
                builder.appendLine("val ref = $rawJnaByValue(com.sun.jna.Memory(maxOf(1L, $rawJnaByValue().size().toLong() * size.toLong())))")
                builder.appendLine("allocator.register(ref) // retain for the scope: ArrayHolder keeps only the raw address, and GC of the Memory would free the native block")
                builder.appendLine("val array = ref.toArray(size.toInt())")
                builder.appendLine("array.forEachIndexed { index, struct ->")
                builder.indent()
                builder.appendLine("struct.clear(); struct.read(); /* JNA toArray auto-reads heap garbage into the Java fields; re-read the zeroed memory so write() cannot restage it */ provider(index.toUInt(), ByValue(struct as $rawJnaByValue)); struct.write()")
                builder.unindent()
                builder.appendLine("}")
    ...
    private fun emitNativeDisplayHandle(decl: Declaration.Scoped) {
        ...
        builder.appendLine("actual companion object {")
        builder.indent()
        builder.appendLine("actual operator fun invoke(address: $nativeAddress): WGPUNativeDisplayHandle = ByReference($rawJnaByReference(address).also { it.read() })")
        ...
        builder.appendLine("allocator.register(ref) // retain for the scope: ArrayHolder keeps only the raw address, and GC of the Memory would free the native block")
        builder.appendLine("val array = ref.toArray(size.toInt())")
        builder.appendLine("array.forEachIndexed { index, struct -> struct.clear(); struct.read(); /* JNA toArray auto-reads heap garbage into the Java fields; re-read the zeroed memory so write() cannot restage it */ provider(index.toUInt(), ByValue(struct as $rawJnaByValue)); struct.write() }")
        builder.appendLine("val pointer = if (size == 0u) com.sun.jna.Pointer.NULL else array.first().pointer")
        builder.appendLine("return $arrayHolder(pointer)")
        builder.unindent()
        builder.appendLine("}")
